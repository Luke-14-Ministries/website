-- 0014: let families edit a registration whose people are already CONFIRMED.
--
-- The registration_participants UPDATE policy deliberately stops families from
-- modifying a confirmed row (status must stay in draft/submitted/cancelled).
-- The submit RPC's blanket upsert touched every row -- so any edit after staff
-- confirmation failed with an RLS violation. Rewritten participant logic:
--   * new person            -> insert as 'submitted' (needs staff review)
--   * role changed          -> update; a CONFIRMED person flips back to
--                              'submitted' so staff re-review the change
--   * nothing changed       -> confirmed rows are left completely untouched
-- The function stays SECURITY INVOKER: family RLS remains the backstop.
...
begin
  ...
  loop
    ...
    -- Participant: insert new as 'submitted'; on a ROLE change, confirmed
    -- people flip back to 'submitted' for staff re-review; otherwise leave
    -- existing rows (especially confirmed ones) untouched.
